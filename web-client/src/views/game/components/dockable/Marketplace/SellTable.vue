<template>
    <v-data-table :headers="headers" :items="getData">
        ...
        <template v-slot:item.actions="{ item }">
            <v-btn @click="sell(item.id, item.quantity)" color="warning">Sell</v-btn>
        </template>
    </v-data-table>
</template>
...
<script lang="ts">
    ...
    export default class SellTable extends Vue {
        @Prop({required: true})
        protected boughtCommodities!: MarketCommodity[];
        ...
        get getData() {
            return this.boughtCommodities.map((x: MarketCommodity) => {
        ...
        protected async sell(itemId: number, quantity: number) {
            const marketCommodity = this.boughtCommodities.find((x: MarketCommodity) => {
                return x.id === itemId;
            }) ?? null;
